...
impl UserManager {
    ...
    pub async fn add_one(&self, user: UserDraft) -> Result<User, ApiError> {
        ...
        let id = result.inserted_id.as_object_id().ok_or_else(|| ApiError::DatabaseError("Id not generated".into()))?;
    ...
    pub async fn remove_one(&self, id: ObjectId) -> Result<User, ApiError> {
        ...
        let user = self.collection.find_one_and_delete(filter, None).await?.ok_or_else(|| ApiError::QueryError(Ressource::User))?;
